  ...
  module Shared
    class CountLogger
      ...
      def count_updated # rubocop:disable Metrics/AbcSize
        ...
        {
          sync_status_attempts: Admin::SyncStatus.where(last_attempted_sync_at: @sync_started_at..).count,
          sync_statuses_updated_to_success:
            Admin::SyncStatus.where(last_successful_sync_at: @sync_started_at..).count,
          sync_statuses_updated_to_failure:
            Admin::SyncStatus.where(last_attempted_sync_at: @sync_started_at..)
      ...
      def count_all
        {
          sync_statuses_total: Admin::SyncStatus.count,
          successful_sync_statuses: Admin::SyncStatus.where.not(last_successful_sync_at: nil).count,
          failed_sync_statuses: Admin::SyncStatus.where(last_successful_sync_at: nil).count,
          spaces: Space.count,
