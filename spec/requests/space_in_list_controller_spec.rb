...
RSpec.describe SpaceInListController, type: :request do
  ...
  it "can add spaces to a list" do
    ...
    post add_to_personal_space_list_space_path(personal_space_list_id: users_space_list.id, space_id: space.id)
    follow_redirect!
  ...
  it "can remove spaces from a list" do
    ...
    expect(users_space_list.spaces.count).to eq(1)
    delete remove_from_personal_space_list_space_path(personal_space_list_id: users_space_list.id, space_id: space.id)
    follow_redirect!
    expect(response).to have_http_status(:success)
    expect(users_space_list.spaces.count).to eq(0)
  end
  ...
  it "can't add spaces to someone else's list" do
    ...
    post add_to_personal_space_list_space_path(personal_space_list_id: someone_elses_space_list.id, space_id: space.id)
