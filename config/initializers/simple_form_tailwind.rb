...
SimpleForm.setup do |config| # rubocop:disable Metrics/BlockLength Layout/LineLength
  ...
  config.wrappers :vertical_boolean, tag: 'div', class: 'mb-4 flex items-start', error_class: '' do |b|
    ...
    b.wrapper tag: 'div', class: 'flex items-center h-5' do |ba|
      ba.use :input, class: 'h-4 w-4 text-lnu-pink focus:ring-lnu-pink border-gray-300 rounded'
    end
    b.wrapper tag: 'div', class: 'ml-3 text-sm' do |bb|
      bb.use :label, class: 'block', error_class: 'ml-2 block text-sm text-gray-900'
  ...
  config.wrappers :vertical_collection,
                  ...
                  item_label_class: 'my-1 ml-3 block text-sm font-medium text-gray-400', tag: 'div', class: 'my-4' do |b| # rubocop:disable Layout/LineLength
    b.use :html5
    b.optional :readonly
    ...
    b.use :input,
          class: 'focus:ring-2 focus:ring-indigo-500 ring-offset-2 h-4 w-4 text-indigo-600 border-gray-300 rounded', error_class: 'text-red-500', valid_class: 'text-green-400' # rubocop:disable Layout/LineLength
  ...
  config.wrappers :vertical_multi_select, tag: 'div', class: 'my-4', error_class: 'f', valid_class: '' do |b|
    ...
    b.wrapper tag: 'div', class: 'inline-flex space-x-1' do |ba|
      ...
      ba.use :input,
             class: 'flex w-auto w-auto shadow appearance-none border border-gray-300
